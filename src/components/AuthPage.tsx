...
const AuthPage: React.FC<AuthPageProps> = ({ onClose, defaultTab = 'login' }) => {
  const { signIn, signUp, signInWithGoogle, checkUsername, isAnonymous, convertAccount } = useAuth();
  ...
  const handleSignupSubmit = async (e: React.FormEvent) => {
    ...
    // 익명 사용자 → 계정 전환 / 신규 가입 분기
    const result = isAnonymous
      ? await convertAccount(signupEmail, signupPassword, username)
    ...
    if (result.error) {
      ...
      setMessage(
        isAnonymous
          ? '확인 메일을 보냈어요. 메일 인증 후 모든 기기에서 볼 수 있어요!'
  ...
  const cardContent = screen === 'signup-form' ? (
  ...
  // 독립 전체화면
  return <div className="auth-page">{cardContent}</div>;
};
